...
function keyColor(isAccidental, isHighlighted) {
  return isAccidental
    ? isHighlighted ? "black" : "rgba(160, 160, 160)"
    : isHighlighted ? "ivory" : "rgb(231, 231, 231)";
}

function keyLabelColor(isAccidental, isHighlighted) {
  return isAccidental
    ? isHighlighted ? "white" : "rgb(231, 231, 231)"
    : isHighlighted ? "black" : "rgb(80, 80, 80)"
}
...
export class PianoKey {
  ...
  writeLabel(ctx, chordRootNumber) {
    ...
    isolate(ctx, () => {
      ...
      this.note.labels.forEach((labelText, ixLabel) => {
        const labelYPosition = textYPosition + (ixLabel * pianoKeyStyles.textHeight);
  ...
  drawShape(ctx, chordRootNumber) {
    ...
    isolate(ctx, () => {
      ctx.beginPath()
      ctx.rect(this.xPosition, 0, pianoKeyStyles.keyWidth, pianoKeyStyles.keyHeight);
